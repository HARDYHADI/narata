...
const CONTENT_TYPE = "DRAMA";

const NAV_ITEMS: ContentNavItem[] = [
  { key: "home", label: "드라마 홈", href: "/dramas/home" },
  { key: "browse", label: "전체 드라마", href: "/dramas" },
  { key: "ongoing", label: "방영 중", href: "/dramas?status=ONGOING" },
  { key: "upcoming", label: "공개 예정", href: "/dramas?status=UPCOMING" },
  { key: "rating", label: "평점 순위", href: "/dramas?sort=rating" },
  { key: "collection", label: "컬렉션", href: "/dramas?view=collection" },
];
...
function buildSortHref(params: SearchParams, sort: MovieSortOption): string {
  ...
  qs.set("sort", sort);
  return `/dramas?${qs.toString()}`;
}

export default async function DramasBrowsePage({
  searchParams,
}: {
  ...
  if (params.view === "collection") {
    return (
      <>
        ...
        />
        <ContentNavBar label="드라마" active="collection" items={NAV_ITEMS} />
  ...
  const runtimeCapped = params.runtime === "60";
  ...
  const filters: MovieFilterParams = {
    ...
    maxRuntime: runtimeCapped ? 60 : undefined,
    sort,
  ...
  return (
    <>
      ...
      />
      <ContentNavBar label="드라마" active="browse" items={NAV_ITEMS} />

      <div className="wrap">
        <div className="page-title">
          <div>
            <span className="eyebrow">DRAMA EXPLORE</span>
            <h1>조건별 드라마 탐색</h1>
        ...
        <div className="browse">
          <form action="/dramas" method="get" className="card filter">
            ...
            <div className="filterset">
              ...
              <div className="checks">
                <label className="check">
                  <input type="checkbox" name="runtime" value="60" defaultChecked={runtimeCapped} />
                  <i />
                  회당 60분 이하
                </label>
              </div>
